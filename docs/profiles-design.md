...
Status: **Proposal** (no code yet). Tracks Item 5 of the product-readiness
feedback. Open questions are flagged in the **Open Questions** section.

This is the design doc. Implementation lands in a follow-up PR after review.
...
1. **Credentials merging across layers.** §5 says the layer that
   produced `provider` also contributes credentials, no merging.
   Alternative: credentials always merge from user profile
   regardless of which layer set the provider. Cleaner for the case
   where a project file says `provider: dropbox` but credentials
   live only in the user file. **Lean: merge** (last-write-wins
   per env-var name, user file is the floor, project file can
   override individual keys).

2. **`SIGN_PROFILE` vs `--profile` precedence.** Currently §5 has
   flag > env. For consistency with `SIGN_PROVIDER` (where the env
   is layer 2 below the flag) this is right. Confirming the
   convention.

3. **Profile schema versioning.** `version: 1` is recorded. When we
   need a v2, do we migrate in place or refuse to load until
   migrated? **Lean: migrate in place** with a `sign profile
   migrate` command if anything non-trivial changes; v1 → v1.x
   additions are backward-compatible by definition.

4. **`sign profile show --json`?** The default `show` output should
   probably be human-readable with the provenance column. A `--json`
   flag for machine consumption is cheap and worth including in v1.

5. **Should `sign profile init` write `sign-profile.json` too,
   or only the user profile?** `init --project` could be a flag.
