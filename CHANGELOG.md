...
- **`sign doctor` preflight** — per-check status report. Each check is `{ name, status: "ok"|"warn"|"fail", message, hint? }`; the wrapping object is `{ ok, checks[] }`. Checks: `node`, `sqlite`, `provider`, `dbPath`, `writable`, `localSignerKey`. Exit `0` if every check is `ok` or `warn`; exit `3` if any check is `fail`. Designed to be the first call in an agent loop — branch on `checks[].name` + `status` and use `hint` for self-recovery. Output is stable JSON on stdout, so `jq -e '.checks[] | select(.status=="fail")'` works.
...
- **Extended audit export bundle** — `audit export` now emits `bundleVersion: 2` in the manifest. New files alongside the existing `audit.json` / `signed.pdf` / `manifest.json`: `original.pdf` (the unsigned source, byte-identical to the request's input), `README.md` (human-readable handoff with the request ID, signer list, and verify commands), `receipts/<signer-email>.json` (per-signer event subsets — Bob's file is guaranteed not to leak Alice's events). All files are sha256'd in `manifest.json`. Backward-compat: `request export-receipt` + `request verify-receipt` still emit/consume bundleVersion 1; the new bundle is opt-in via `audit export`.
- **Trust labels in signed-PDF inspection** — `request verify-signed-pdf --inspect` now adds a `trust` field to every entry in `signatures[].signers[]`. Values: `"trusted"` (cert chains to a trusted root in `data/local-trust-store/`), `"untrusted-self-signed"` (valid signature but no chain to trust), `"unverified"` (chain validation failed), `"expired"` (cert expired at signing time), `"unknown"` (cert parse error). The summary also gains `worstTrust` so an agent can branch on a single field instead of scanning every signer.
...
- **[`docs/agent-guide.md`](docs/agent-guide.md)** — canonical agent reference. Output conventions (stdout JSON / stderr error envelope), the cross-command exit-code map, per-command schemas + side-effects + idempotency for the new surfaces (`doctor`, `pdf stamp verify`, `workflow nda`, `audit export` v2, trust labels, strict-provider), and the failure-mode → recovery table.
- **[`docs/recipes/preflight.md`](docs/recipes/preflight.md)** — pre-production agent recipe: `doctor` → strict-provider → `pdf stamp verify` in CI → `audit export` for handoff. Shows exit-code branching at each step.
