...
| `AUTO_PLACE_AMBIGUOUS` on `sign sign --auto-place true` | Multiple high-confidence (`≥0.8`) candidates were found — the detector refuses to pick one because it can't tell which signature line is meant for this signer. | Inspect `details.candidates` in the error JSON, pick the rectangle for this signer, and pass it as `--image-page/--image-x/--image-y/--image-width/--image-height`. Alternatively, run `sign pdf detect-signature-field --pdf <path>` first to view the full candidate list. |
