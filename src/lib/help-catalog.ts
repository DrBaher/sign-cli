...
export const SIGN_CLI_VERSION = "0.6.5";
...
export const HELP_CATALOG: CommandSpec[] = [
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "--rate-limit", description: "Tokens per second per IP (token bucket). Over-budget requests get 429 with Retry-After. Keys on the socket peer address unless --trust-proxy is set." },
      { name: "--rate-limit-burst", description: "Bucket capacity (max burst). Defaults to 2× --rate-limit." },
      { name: "--trust-proxy", description: "true to key rate-limiting on the X-Forwarded-For header. Only enable behind a reverse proxy you control that sets it — otherwise clients can spoof the header to evade rate limiting." },
