...
function makeInMemoryFakePg(): PgQueryable {
  ...
  return {
    async query(text, params = []) {
      const sql = text.trim();
      // DDL — accept silently.
      if (/^(CREATE|DROP|REPLACE|ALTER|--)/i.test(sql) ||
      ...
      if (/^INSERT INTO audit_events/i.test(sql)) {
        const [request_id, event_type, payload_json, hash_prev, hash_self, hash_algo, created_at] = params as unknown[];
        tables.audit_events.push({ id: nextEventId++, request_id, event_type, payload_json, hash_prev, hash_self, hash_algo, created_at });
        return { rows: [], rowCount: 1 };
      }
      ...
      if (/^SELECT id, request_id, event_type, payload_json, hash_prev, hash_self, hash_algo, created_at\s+FROM audit_events\s+WHERE request_id = \$1\s+ORDER BY id ASC/i.test(sql)) {
        const requestId = (params as unknown[])[0];
        const rows = tables.audit_events.filter((r) => r.request_id === requestId);
        return { rows, rowCount: rows.length };
      }
      if (/^SELECT id, event_type, payload_json, hash_prev, hash_self, hash_algo, created_at\s+FROM audit_events\s+WHERE request_id = \$1\s+ORDER BY id ASC/i.test(sql)) {
        const requestId = (params as unknown[])[0];
        const rows = tables.audit_events
          .filter((r) => r.request_id === requestId)
          .map(({ id, event_type, payload_json, hash_prev, hash_self, hash_algo, created_at }) => ({ id, event_type, payload_json, hash_prev, hash_self, hash_algo, created_at }));
        return { rows, rowCount: rows.length };
      }
